...
/* find the index to PF file table entry whose "filename" field is same as
 * "filename" */
...
PFreadfunc(int fd, int pagenum, PFpage *buf) {
  ...
  if ((error_value = lseek(PFftab[fd].unixfd,
                           pagenum * sizeof(PFpage) + PF_HDR_SIZE)) == -1) {
    PFerrno = PFE_UNIX;
    return (PFerrno);
  }
...
#define PFinvalidFd(fd) \
  ((fd) < 0 || (fd) >= PF_FTAB_SIZE || PFftab[fd].fname == NULL)

/* true if page number of file is invalid in case of ->
 0 or >= number of pages in the file */
#define PFinvalidPagenum(fd, pagenum) \
  ((pagenum) < 0 || (pagenum) >= PFftab[fd].hdr.numpages)
...
/* destroy the file filename. the file shoudl have existed and should not be
 * opened */
...
int PF_AllocPage(int fd, int *pagenum, char **pagebuf) {
  ...
  if (PFftab[fd].hdr.firstfree != PF_PAGE_LIST_END) {
    ...
    *pagenum = PFftab[fd].hdr.numpages;
    if ((error_value = BF_AllocBuf(fd, *pagenum, &fpage, PFwritefunc)) !=
        PFE_OK)
...
int PF_GetNextPage(int fd, int *pagenum, char **pagebuf) {
  ...
  for (tmppage = *pagenum + 1; tmppage < PFftab[fd].hdr.numpages; tmppage++) {
    if ((error_value =
             BF_GetBuf(fd, tmppage, &fpage, PFreadfunc, PFwritefunc)) != PFE_OK)
      return (error_value);
...
int PF_GetThisPage(int fd, int pagenum, char **pagebuf) {
  ...
  if ((error_value = BF_GetBuf(fd, pagenum, &fpage, PFreadfunc, PFwritefunc)) !=
      PFE_OK)
...
/*after checking the validity of the fd and pagenum values, this function marks
 * the page associated with fd and pagenum */
...
/*after checking the validity of the fd and pagenum values, this function marks
 * the dirty associated with fd and pagenum  */
...
static char *PFerrormsg[] = {
    "No error", "No memory", "No buffer space", "unix error",
    "incomplete read of page from file", "incomplete write of page to file",
    "incomplete read of header from file",
    "incomplete write of header from file", "invalid page number",
    "file already open", "file table full", "invalid file descriptor",
    "end of file", "page already free", "page already unfixed",
    "new page to be allocated already in buffer", "hash table entry not found",
