...
struct leaf_nodes{		/*an array of info about leaf nodes*/
	int pagenum;
	int index;
	int array[100];  /*change it!*/ /*array of entries*/
};
...
/*#define two_p(pagebuf, i, value, attrLength)\
	...
	static int leaf_node(int AM_fd)
	{
		int err;
		int leaf_node;
		char *pagebuf;
...
static int insert_value_into_node(int AM_fd, int pagenum_left, void *value, int node, char attrType, int attrLength, int *pagenum_right, void *val)
{
	...
	char *pagebuf;			/*node*/
	int m = -1;
	char *new_node = (char*)&m;   /* *new node = -1 */
...
*/				free(leaf);
		...
		num_records = ((struct node_header*)pagebuf)->numRecords;
...
void AM_Init(void)
{
	...
	/* Initialize HF layer*/
 	...
 	for (i = 0; i < MAXOPENFILES; i++) /*this function is needed??*/
	 {
...
/*-----Create, Open, Allocate PF File------------------------------*/
	...
	/*if (!(empty_root_node == FALSE) return AME_ROOTNULL; */ /*is it proper return parameter??*/
...
int AM_InsertEntry(int AM_fd, char *value, /*RECID*/ int recId)
{
	...
	attrLength =( ((struct node_header*)&pagebuf)->attrLength );  /*check if it is ok (memory size without malloc) &pagebuf..*/
...
/*-----------FIND LEAF-------------------------------------*/
	...
	if ( (err = find_leaf(AM_fd, value,  attrLength,((struct node_header *)&pagebuf)->attrType, &leaf)) != AME_OK) 
	{
		AMerrno = err;
		return err;
	}
...
/*-------------------------------*/
	...
	for ( i = 0; i < num_records(attrLength); i++) /* while next record != 0 //scan table is wrong.??.ocrrect!*/
	{	
...
/*check v and p parameters*/
	...
	err = PF_UnpinPage(AM_fd, leaf->pagenum, TRUE);
	free(leaf);
	return err;
...
/*RECID*/ int AM_FindNextEntry(int scanDesc)
{
	...
	attrLength = ((struct node_header *)&pagebuf)->attrLength; /* without & - a problem, unitialized function*/
	

	table_scan = &AM_scan_table[scanDesc];
	
	/*Check if a scan is valid*/
	if( (scanDesc < 0) || (scanDesc >= MAXSCANS) || (!table_scan->is_empty) )/*check this condition!*/
	{
...
/*--------------------------------------------*/
	...
	while(1)
	{
		if( (err = PF_GetThisPage(table_scan->fd, table_scan->pageId, &pagebuf) != PFE_OK) )
			{
				AMerrno = err;
				return err;
			}
	...
	last node
	unpin page?*/
		for ( (table_scan->index)++; (table_scan->index) < num_records(attrLength); (table_scan->index)++)
		{
	...
	/* the end of node (last node) */
		if(table_scan->index == num_records(attrLength) ) /*check!  index == a record is a pair <v, p>*/
		{
			...
			if( (err = PF_UnpinPage(table_scan->fd, table_scan->pageId, FALSE) != PFE_OK) )/* unpin block*/
			{
				AMerrno = err;
				return err;
			}
			...
			table_scan->pageId =next_leaf_page;
			table_scan->index =-1;
			continue;	
		}
		/*TODO with a pair of <p , v> ???*/
		recId = node_p(pagebuf,table_scan->index, table_scan->attrLength);
		...
		if( (err = PF_UnpinPage(table_scan->fd, table_scan->pageId, FALSE) != PFE_OK) )
		{
			AMerrno = err;
			return err;
		} 
